"""
This script is intended to be used to create new users in BANPRD
"""
import argparse
...
PARSER = argparse.ArgumentParser(prog="BANPRD User Generator")
...
class BANPRDGenerator(OracleDB):
    """
    Class used to interface with BANPRD
    """
    ...
    def create_user(self):
        ...
        statements = (
            ...
            IDENTIFIED BY {RANDOMPASSWORD} PROFILE USERS
    ...
    def create_service_account(self):
        ...
        statements = (
            f"""
                create user {USER} identified by {RANDOMPASSWORD};
...
if __name__ == "__main__":
    BG = BANPRDGenerator(USER, DATABASE)
    if ARGS.svc:
        BG.create_service_account()
    else:
        BG.create_user()

    if ROLES:
        BG.indiv_roles(USER, ROLES)
    if MIRRORED_USER:
        BG.mirror_from(MIRRORED_USER)
